...
local browser_class = "^(" .. table.concat(browser_list, "|") .. ")$"
local browser_set = {}
...
window("polkit-agents-centered",
    {
        class =
        "^(polkit-gnome-authentication-agent-1|org\\.gnome\\.PolicyKit1\\.AuthenticationAgent|lxpolkit|kdesu|kdesudo|org\\.kde\\.kdesu)$"
    },
    {
        float = true,
        center = true,
...
window("pinentry-centered",
    {
        class =
        "^(pinentry|pinentry-gtk|pinentry-gnome3|pinentry-qt|pinentry-qt5|gcr-prompter|gcr-viewer|gnome-keyring-prompt)$"
    },
    {
        float = true,
        center = true,
...
window("update-managers-centered",
    { class = "^(update-manager|gnome-software|pamac-manager|pamac|octopi|discover|mintupdate|muon|synaptic|gdebi)$" }, {
        float = true,
        center = true,
    })

-- GNOME control center and system settings (gnome-control-center is tiled, see above)
window("system-settings-centered",
    {
        class =
        "^(unity-control-center|cinnamon-settings|xfce4-settings-manager|systemsettings5|systemsettings|plasma-settings)$"
    },
    {
        float = true,
        center = true,
...
-- Network / wifi / VPN managers (nm-connection-editor is tiled, see above)
window("network-managers-centered", { class = "^(nm-applet|wicd-gtk|wpa_gui|vpn-ui)$" }, {
    float = true,
    center = true,
})
...
window("font-theme-choosers-centered",
    { class = "^(font-manager|gnome-font-viewer|lxappearance|nwg-look|kvantum|qt5ct|qt6ct)$" }, {
        float = true,
        center = true,
    })
...
-- Covers extension popups, Bitwarden autofill/save prompts, permission
-- requests, and any other small browser-spawned dialog. Patterns already
-- present in `centered_popups` (e.g. Bitwarden, Passkey, WebAuthn, FIDO,
-- Security Key, Authenticator, Permission, Access Request) are omitted here
-- since that list has no class restriction and already matches browsers.
